...
func Test_Recipe(t *testing.T) {
	r := Recipe{CalcHash([]byte("hello")), CalcHash([]byte("world"))}
	...
	if j, err := json.Marshal(r); err != nil {
		t.Errorf("failed to marshal recipe; %s", err.Error())
	} else if string(j) != except {
		t.Errorf("failed to marshal recipe; except %v but got %v", except, string(j))
	}

	var r2 Recipe
	if err := json.Unmarshal([]byte(except), &r2); err != nil {
		t.Errorf("failed to unmarshal recipe; %s", err.Error())
	} else if len(r2) != len(r) {
		t.Errorf("unmarshaled recipe has unexcepted data; excepted %v but got %v", r, r2)
	} else {
		for i, x := range r2 {
			if r[i] != x {
				t.Errorf("unmarshaled recipe has unexcepted data; excepted %v but got %v", r, r2)
