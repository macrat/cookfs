...
func Test_Recipie(t *testing.T) {
	r := cookfs.Recipie{"/tag/to/recipie", []cookfs.Hash{cookfs.CalcHash([]byte("hello")), cookfs.CalcHash([]byte("world"))}}
	...
	var r2 cookfs.Recipie
	if err := json.Unmarshal([]byte(except), &r2); err != nil {
		t.Errorf("failed to unmarshal recipie; %s", err.Error())
	} else if r2.Tag != "" {
		t.Errorf("unmarshaled recipie has unexcepted tag; got %s", r2.Tag)
	} else if len(r2.Data) != len(r.Data) {
		t.Errorf("unmarshaled recipie has unexcepted data; excepted %v but got %v", r.Data, r2.Data)
	} else {
		for i, x := range r2.Data {
			if r.Data[i] != x {
				t.Errorf("unmarshaled recipie has unexcepted data; excepted %v but got %v", r.Data, r2.Data)
			}
		}
